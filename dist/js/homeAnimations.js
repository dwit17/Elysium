...
(function () {
  ...
  /**
   * 6. SECTION 6 — TRUST & VOICE (SCROLL-DRIVEN WORD HIGHLIGHT REVEAL)
   * Locks in at top of viewport, reveals text word-by-word progressively from light gray -> deep black on scroll.
   * Fully responsive across desktop, tablet, and mobile with prefers-reduced-motion support.
   */
  function initTrustVoiceSection() {
    ...
    const words = Array.from(quoteElement.querySelectorAll('.trust-word'));
    ...
    // Initial state: dim stone gray (#57534e)
    gsap.set(words, { color: '#57534e', opacity: 0.6 });
    if (attribution) gsap.set(attribution, { opacity: 0, y: 15 });
    if (eyebrow) gsap.set(eyebrow, { opacity: 0.8 });

    // Timeline pinned scroll scrub
    ...
    // Word-by-word reveal: transition color from dim gray (#57534e) -> pure white (#ffffff)
    trustTl.to(words, {
      color: '#ffffff',
      opacity: 1,
      stagger: 0.08,
    ...
    // Animate attribution in at final phase of scroll
    if (attribution) {
      trustTl.to(
        attribution,
        {
    ...
    console.log('[Elysium Motion] Section 6 (Trust & Voice) word-by-word scroll reveal initialized.');
  }
